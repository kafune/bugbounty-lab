...
- T1068
---
> ⚠️ **SCOPE-GUARD (lab bugbounty) — referência técnica externa, NÃO é skill ativa.**
> Antes de qualquer sonda/payload daqui contra um alvo, valide o host:
> `bash bin/scope-check.sh <host> <handle>` (exit 0 = pode; exit 1 = pare).
> Só toque em host de `targets/<handle>/scope.txt` fora do `out-of-scope.txt`.
> Mutação para fora (upload/POST/escrita) = pedir confirmação ao usuário antes.
> Classes sem impacto (open-redirect, CSP, WAF-bypass unauth, DoS) não valem sessão H1.
> Procedência: mukul975/Anthropic-Cybersecurity-Skills (Apache-2.0). Fundação — não editar.
